...
import (
	...
	"github.com/kcproxy/fami-node/api/panel"
	"github.com/kcproxy/fami-node/common/file"
)
...
func NewLego(info *panel.NodeInfo) (*Lego, error) {
	certFile := filepath.Join("/etc/fami-node/", info.Type+strconv.Itoa(info.Id)+".cer")
	//keyFile := filepath.Join("/etc/fami-node/", info.Type+strconv.Itoa(info.Id)+".key")
	user, err := NewLegoUser(path.Join(path.Dir(certFile),
		"user",
		fmt.Sprintf("user-%s.json", "minode@fami.dev")),
		"minode@fami.dev")
	if err != nil {
...
func (l *Lego) RenewCert() error {
	certFile := filepath.Join("/etc/fami-node/", l.info.Type+strconv.Itoa(l.info.Id)+".cer")
	//keyFile := filepath.Join("/etc/fami-node/", info.Type+strconv.Itoa(info.Id)+".key")
...
func (l *Lego) writeCert(certificates *certificate.Resource) error {
	certFile := filepath.Join("/etc/fami-node/", l.info.Type+strconv.Itoa(l.info.Id)+".cer")
	keyFile := filepath.Join("/etc/fami-node/", l.info.Type+strconv.Itoa(l.info.Id)+".key")
