		...
		default:
			...
			in.StreamSetting.TLSSettings = &coreConf.TLSConfig{
				Certs: []*coreConf.TLSCertConfig{
					{
						CertFile: filepath.Join("/etc/fami-node/", nodeInfo.Type+strconv.Itoa(nodeInfo.Id)+".cer"),
						KeyFile:  filepath.Join("/etc/fami-node/", nodeInfo.Type+strconv.Itoa(nodeInfo.Id)+".key"),
					},
				},
			}
		}
